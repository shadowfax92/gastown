...
|------|---------|------------|
| Senior Engineer (zombie-scan) | `file-warrant`, `no-op`, `nudge`, `log-and-watch`, `escalate-to-product manager`, `create-cleanup-wisp` | `senior engineer-zombie.yaml`, `class-a-senior engineer.yaml` |
| Senior Engineer (plugin-run) | `execute-plugin`, `skip` | `senior engineer-plugin-gate.yaml` |
| Senior Engineer (dog-health) | `no-op`, `log-and-watch`, `file-warrant`, `force-clear`, `spawn-dog`, `retire-dog` | `senior engineer-dog-health.yaml`, `class-a-senior engineer.yaml` |
| QA Engineer | `no-op`, `nudge`, `escalate`, `nuke`, `mark-zombie`, `create-cleanup-wisp` | `QA engineer-stuck.yaml`, `QA engineer-cleanup.yaml`, `class-a-QA engineer.yaml` |
| Release Engineer | `reject-mr`, `file-ticket-and-proceed`, `retry`, `skip-mr`, `investigate` | `release engineer-triage.yaml`, `release engineer-conflict.yaml`, `class-a-release engineer.yaml` |
...
### Ticket metadata labels change

Shell output in test cases contains ticket JSON with labels like `agent_state:running`, `agent_state:idle`. If these label names change in `bd` or Gas Town agent code, update the simulated shell output in affected tests.
...
|---------|---------|---------|
| Agent worktree | `git -C /town/gastown/agents/<name>` | senior engineer, QA engineer, dog tests |
| Tmux session | `tmux has-session -t bd-agent-<name>` | senior engineer, QA engineer tests |
| Ticket commands | `bd show agent-<name> --json` | all role tests |
| Mail commands | `gt mail list --to agent-<name>` | QA engineer tests |

If directory structure, tmux naming, or CLI interfaces change, search and update:

```bash
grep -r '/town/gastown/agents/' tests/
grep -r 'bd-agent-' tests/
...
Some test descriptions encode timing expectations (e.g., "10 minutes idle tfeaturegers nudge", "45 minutes tfeaturegers escalate"). These are embedded in test case descriptions and `context` fields, not in a config file. If patrol formulas adjust timing thresholds, review the test scenarios to ensure they still test the featureht behavior.
...
|-------------|-------------------|---------|
| `spawn-dog` | `gt dog add` | Senior Engineer dog pool maintenance |
| `retire-dog` | `gt dog remove` | Senior Engineer dog pool maintenance |
| `force-clear` | `gt dog clear --force` | Senior Engineer dog health check |
| `file-warrant` | `bd create --type=warrant ...` | Senior Engineer zombie detection |
| `create-cleanup-wisp` | `bd create --type=wisp ...` | Senior Engineer/QA engineer cleanup |
