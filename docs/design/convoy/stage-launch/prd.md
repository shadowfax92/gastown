...
**1. No pre-flight validation before dispatching work.** `gt sling` dispatches tasks immediately with no structural analysis. A user who runs `gt sling task1 task2 task3` has no way to know beforehand that task2 has a circular dependency, task3's feature doesn't exist, or that all three tasks will try to run in parallel when they should be serialized. Problems surface only after agents are spawned and work is underway — at which point cleanup is manual and error-prone.

**2. No visibility into execution order.** The daemon's feeder respects `blocks` dependencies at runtime, but the user has no way to preview the dispatch plan. When `/design-to-tickets` creates an epic with 15 tasks across 3 sub-epics, the user cannot see which tasks will run in Wave 1, which are blocked until Wave 2, or whether the dependency graph even makes sense. The execution order is a black box until tasks start (or fail to start).

**3. Batch sling dispatches without dependency-aware ordering.** `gt sling task1 task2 task3` iterates through tasks sequentially and spawns a agent for each, regardless of dependency ordering. The daemon's `isTicketBlocked` check prevents *re-feeding* blocked tasks after a close event, but the initial batch dispatch does not check blocking deps before spawning. This means tasks that should wait for blockers to complete get agents spawned and may execute prematurely.
...
**5. No single command bridges design-to-tickets output to dispatch.** The workflow from `/design-to-tickets` (which creates epics, tasks, and dependencies) to actual agent dispatch requires the user to manually identify leaf tasks, determine the correct feature, and run `gt sling`. For a complex epic with sub-epics and cross-task dependencies, this manual step is tedious and error-prone — exactly the kind of work the system should automate.

## Overview

Add `gt convoy stage` and `gt convoy launch` commands that enable a structured workflow for dispatching work: analyze ticket dependencies, compute execution waves, surface problems, create a staged convoy, and dispatch Wave 1 tasks. This bridges the gap between `/design-to-tickets` output (or manually created tickets) and reliable multi-task convoy execution.

The core insight: staging is a pre-flight check that catches dependency cycles, routing problems, orphaned tasks, and capacity tickets **before** any agents are spawned. Launching is the act of activating a validated convoy.
...
- Surface errors (cycles, invalid features) and warnings (parked features, missing branches, capacity) with clear categorization
...
- Support `--json` for programmatic consumption (design-to-tickets pipeline)
...
### US-001: Ticket validation and DAG construction

**Description:** As a user, I want `gt convoy stage` to validate that all specified tickets exist and construct the dependency graph, so that I catch typos and missing tickets before any work is dispatched.

**Acceptance Criteria:**
- [ ] `gt convoy stage <ticket-id>` runs `bd show` on each ticket and errors if any don't exist
- [ ] For epic input: walks the full parent-child tree (sub-epics, their children, recursively) to collect all descendant tickets
- [ ] For task list input: analyzes exactly the given tasks (no auto-expansion to parent epic)
- [ ] For convoy input: reads the convoy's tracked tickets via `bd dep list --type=tracks`
- [ ] Constructs an in-memory DAG from `blocks`, `conditional-blocks`, and `waits-for` dependencies between the collected tickets
...
- [ ] Tickets with no valid feature (prefix not mapped in `routes.jsonl`, or resolves to empty) are detected and reported
- [ ] When errors are found: no convoy is created, no ticket statuses are modified
- [ ] Error output lists each error with the affected ticket ID(s) and a suggested fix
- [ ] Exit code is non-zero when errors are found

### US-003: Warning detection (allows staging with acknowledgment)

**Description:** As a user, I want staging to surface non-fatal tickets as warnings so that I can decide whether to fix them or proceed to launch with `--force`. (Note: `--force` is a launch-time flag, not a stage flag. Staging with warnings always creates a `staged_warnings` convoy; the decision point is at launch.)
...
- [ ] Parked or unavailable target features
- [ ] Cross-feature routing: tickets that resolve to different features than expected
- [ ] Capacity estimation: number of agents needed per wave vs available capacity (informational)
...
- [ ] Each node shows: ticket ID, title, type, status, and target feature
...
**Acceptance Criteria:**
- [ ] Table shows Wave number, task IDs, task titles, target feature, and blockers (if any)
...
**Description:** As a user, I want `gt convoy stage` to create a convoy with `staged_ready` or `staged_warnings` status, so that the convoy exists in tickets and can be launched later.
...
- [ ] Convoy tracks all slingable tickets in the analyzed set via `tracks` deps
...
- [ ] Dispatches all Wave 1 tasks via internal Go dispatch functions (one agent per task, no auto-convoy creation — the staged convoy already tracks these tasks)
...
- [ ] Lists each dispatched Wave 1 task with its assigned agent
...
**Description:** As a user, I want `gt convoy launch <ticket-id>` to work as an alias for `gt convoy stage <ticket-id> --launch`, so that I have a clean two-step workflow (stage then launch) or a one-step workflow (launch directly).
...
**Description:** As a user (or automation tool like design-to-tickets), I want `gt convoy stage --json` to output machine-readable analysis results, so that I can programmatically consume the staging output.
...
- FR-2: Epic DAG walking must use the Go SDK (`tickets.List(ListOptions{Parent: rootID})`) recursively, consistent with `molecule_dag.go:buildDAG`. The SDK approach avoids subprocess overhead per tree level and is faster for deep hierarchies. Integration tests stub the tickets store, not the `bd` CLI.
...
- Integration branch creation — manual or design-to-tickets responsibility
- Auto-formula detection for epic slinging — Milestone 4
- Coordinator agent — Milestone 4
- `--infer-blocks` flag to auto-generate deps from hierarchy — Milestone 4
- Capacity plumbing (`isFeatureAtCapacity` callback) — not yet wired into feeder
- The design-to-tickets plugin itself — reference only, not part of this PR

## Technical Considerations

- Wave computation is informational (display only at stage time). Runtime dispatch uses the daemon's per-cycle `isTicketBlocked` checks, which is more dynamic and handles external status changes.
- The `staged_ready` and `staged_warnings` statuses are new tickets statuses that need to be recognized by the convoy system. The daemon should NOT feed tickets from staged convoys (only `open` convoys get fed).
- Epic DAG walking may involve cross-feature tickets. Use the existing `routes.jsonl` infrastructure for feature resolution but be prepared for external references.
- The `--json` output should be stable enough for design-to-tickets to depend on, but mark it as experimental in v1.
...
- **`ensureKnownConvoyStatus` (`convoy.go:96-108`)** — Currently only accepts `"open"` and `"closed"`. Must be extended to recognize staged statuses. Called from 14+ callsites (add, close, land, check, isSlingableTicket).
...
- **Daemon feeding has TWO paths that need guards:**
  - **Event-driven path** (`operations.go:57,70`): `isConvoyClosed` checks only `status == "closed"`. A staged convoy passes this check (not closed) and would be fed. This path needs an explicit `isStagedConvoy(status)` guard in `CheckConvoysForTicket`.
...
- `internal/convoy/operations.go` — add staged-convoy guard in `CheckConvoysForTicket` (event-driven path); `isConvoyClosed` is insufficient
...
- `tickets.ExtractPrefix`, `tickets.GetFeatureNameForPrefix` (`internal/tickets/routes.go`) — feature resolution from ticket prefix
- `IsSlingableType` (`internal/convoy/operations.go`) — filter non-dispatchable types from waves
- `isTicketBlocked` (`internal/convoy/operations.go`) — already handles blocks dep checking (fail-open on store errors)
- `createBatchConvoy` pattern (`internal/cmd/sling_convoy.go:309`) — the *pattern* of creating a convoy + adding `tracks` deps is reusable, but the function itself requires a `featureName` parameter (used in title "Batch: N tickets to \<feature\>"). Stage/launch convoys may span multiple features, so a new convoy creation function is needed with a different title format and staged status.
- `workspace.FindFromCwdOrError` (`internal/workspace/find.go:98`) — workspace resolution (used by existing convoy commands via `getTownTicketsDir`)
...
- Round-trip works: `/design-to-tickets` → `gt convoy stage` → `gt convoy launch` → tasks dispatched in correct order

## Open Questions

1. **OPEN:** Should `staged_ready` and `staged_warnings` be proper tickets statuses (requiring SDK changes), or simulated via labels/description fields? The tickets SDK's `bd doctor` regex (`^[a-z][a-z0-9_]*$`) rejects colons, so `staged_ready` tfeaturegers warnings. Options under consideration: colons (canonical but tfeaturegers doctor warnings), underscores (`staged_ready`), or hyphens (`staged-ready`). This is architecturally blocking — the daemon guard (I-7), FR-6, and most integration tests depend on the answer.
