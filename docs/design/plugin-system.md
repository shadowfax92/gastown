...
> Written 2026-01-11, engineers/george session.

## Problem Statement

Gas Town needs extensible, project-specific automation that runs during Senior Engineer patrol cycles. The immediate use case is rebuilding stale binaries (gt, bd, wv), but the pattern generalizes to any periodic maintenance task.
...
The Senior Engineer (agent) evaluates gates and decides whether to dispatch. Go code provides transport (`gt dog dispatch`) but doesn't make decisions.
...
**Feature-level** (`<feature>/plugins/`): Project-specific plugins.

The Senior Engineer scans both locations during patrol.

### Execution Model: Dog Dispatch

**Key insight**: Plugin execution should not block Senior Engineer patrol.

Dogs are reusable workers designed for infrastructure tasks. Plugin execution is dispatched to dogs:

```
Senior Engineer Patrol                    Dog Worker
─────────────────               ─────────────────
1. Scan plugins
2. Evaluate gates
3. For open gates:
...
Benefits:
- Senior Engineer stays responsive
...
Creates: `Plugin Digest 2026-01-10` ticket with summary
...
| `event` | `on = "startup"` | Run on Senior Engineer startup |
...
3. **Plugin scanning** - Senior Engineer scans town + feature plugin dirs
...
14. **Documentation** - So Tickets/Wyvern can create theirs

---

## Open Questions

1. **Plugin discovery in multiple clones**: If gastown has engineers/george, engineers/max, engineers/joe - which clone's plugins/ dir is canonical? Probably: scan all, dedupe by name, prefer feature-root if exists.
...
4. **Plugin disable/enable**: How to temporarily disable a plugin without deleting it? Label on a plugin ticket? `enabled = false` in frontmatter?
...
- mol-senior engineer-patrol.formula.toml - Patrol step plugin-run
