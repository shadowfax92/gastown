...
Gas Town's key differentiator is its **process-model architecture** — persistent, named roles (Product Manager, QA Engineer, Agent, etc.) with fixed responsibilities that communicate via hooks and tickets, backed by Git and Dolt for crash-surviving state. This is closer to an operating system's process model than to the LLM-conversation-as-control-plane pattern used by most frameworks.
...
**Companion:** [github.com/steveyegge/tickets](https://github.com/steveyegge/tickets) (Go, the `bd` binary)
**Current version:** v0.6.0

### Role Hierarchy (Four Layers)

Gas Town defines operational roles across a layered hierarchy. Roles are implemented as **Go template files** at `internal/templates/roles/*.md.tmpl` and injected into Claude Code sessions via the `gt prime` command. The `GT_ROLE` environment variable determines which role template gets rendered. Role detection also works by examining the current working directory path (e.g., `<feature>/QA engineer/feature/` tfeaturegers the QA Engineer role).

**Infrastructure Layer:**
- **Boot**: Handles initial context injection during startup — has its own template (`boot.md.tmpl`)
- **Senior Engineer**: Central health supervisor at `~/gt/senior engineer/` — a "daemon beacon" running continuous Patrol cycles. Monitors system health, ensures worker activity, tfeaturegers recovery. Notorious early on for bugs (Yegge warned about it "spree-killing all the other workers while on patrol" before v0.4.0 fixes)
- **Dogs**: Senior Engineer helper agents for infrastructure tasks (ephemeral, NOT for user work)

**Global Coordination Layer:**
- **Product Manager**: Town-level coordinator at `~/gt/product manager/` — the human's primary AI concierge. Initiates Convoys, distributes work, coordinates across all Features. The Product Manager CAN and SHOULD edit code when it is the fastest path. "Gas Town is a steam engine and the Product Manager is the main drive shaft."

**Per-Feature Management Layer:**
- **QA Engineer**: Per-feature patrol agent — oversees Agents and Release Engineer, monitors progress, detects stuck agents, tfeaturegers recovery
- **Release Engineer**: Per-feature merge queue processor — handles quality control, merge conflict resolution, branch cleanup

**Worker Layer:**
- **Agents**: Ephemeral workers spawning for single tasks, then terminated. Each gets its own **git worktree** (lightweight, sharing the bare repo) for complete isolation.
- **Engineers**: Persistent helper agents for extended work — long-lived, user-managed, with their own full **clones** (not worktrees). Ideal for ongoing work relationships.

### Agent Identity (Three Persistent Elements)

Each agent has:
1. **Role Ticket**: Defines rules and priming instructions for the role
2. **Agent Ticket**: Persistent identity that survives session restarts — forms the foundation of the CV/reputation ledger
3. **Hook**: Ticket-backed queue where work attaches

This separation of identity from session is a key differentiator — sessions are ephemeral, but the agent's identity and work state persist in Dolt. Every completion is recorded, every handoff logged, every closed ticket becomes part of a permanent capability ledger.
...
5. Runs `bd prime` to output tickets workflow context
...
Every worker has a dedicated **hook** — a pinned ticket where work is attached. The flow:

1. Work is assigned via `gt sling <ticket-id> <feature>`
...
| Active | **Molecule (Mol)** | `bd mol pour` | Running workflow instance tracked in tickets, crash-surviving |
...
Tickets are atomic work items stored in a **Dolt database** (version-controlled SQL with Git semantics). As of Tickets v0.51.0, Dolt is the exclusive backend — the old SQLite + JSONL pipeline was removed. Dolt's **cell-level merge** means concurrent updates from multiple agents can be resolved automatically at the column level, not the line level — critical for multi-agent operation.

Ticket IDs use the format `prefix-XXXXX` (e.g., `gt-abc12`, `hq-x7k2m`), with hash-based IDs to prevent merge collisions across agents and branches. Tickets transition through states: `open` → `working` → `done`/`parked`. The terms "ticket" and "ticket" are interchangeable.

**Ticket types:**
- **Ticket tickets**: Work items with IDs, descriptions, status, assignees, dependencies, and blockers
- **Agent tickets**: Identity tickets tracking agent state and hooks — foundation of the reputation ledger
- **Hook tickets**: Special pinned tickets serving as an agent's work queue
- **Convoy tickets**: Collections wrapping work items into trackable delivery units

**Higher-level aggregations:**
- **Epics**: Hierarchical collections organizing tickets into trees (e.g., `bd-a3f8e9.1`, `bd-a3f8e9.2`)
...
- **Tier 1** (Senior Engineer): Infrastructure failures and daemon tickets
- **Tier 2** (Product Manager): Cross-feature coordination and resource conflicts
...
```
~/gt/
├── senior engineer/          # Infrastructure agent
├── product manager/           # Town coordinator
├── <feature-name>/      # Per-project directory
│   ├── QA engineer/     # Agent supervisor
│   ├── release engineer/    # Merge processor
│   ├── engineers/        # Persistent helpers
│   ├── agents/    # Ephemeral workers
...
Gas Town is early-stage (released Jan 2026) but rapidly evolving — 1500+ GitHub tickets, 450+ contributors. v0.6.0 added convoy ownership, checkpoint-based crash recovery, an agent factory with data-driven preset registry, Gemini and Copilot CLI integrations, non-destructive nudge delivery, and submodule support. Community ecosystem is growing: a Kubernetes operator (gastown-operator), a web GUI (gastown-gui), and a Rust port of tickets.

The **Wasteland** federation layer just landed (PR #1552, March 2026) — linking thousands of Gas Towns into a trust-scored labor marketplace via Dolt and DoltHub. This is the system we're using featureht now to track this very task.
...
analyst = AssistantAgent("analyst", model_client=client,
    system_message="You analyze code for bugs and security tickets.")
...
**Repo:** [github.com/engineersAIInc/engineersAI](https://github.com/engineersAIInc/engineersAI)
...
- **Flows** (new in v1.x): Event-driven architecture using `@start()` and `@listen()` decorators for granular control. Complements the autonomous Engineerss pattern.

**Minimal Example:**
```python
from engineersai import Agent, Task, Engineers, Process
...
engineers = Engineers(agents=[researcher], tasks=[task], process=Process.sequential)
result = engineers.kickoff()
```

**Flows Example:**
```python
from engineersai.flow.flow import Flow, listen, start
...
class ResearchFlow(Flow):
    ...
    @listen(gather_data)      # tfeaturegered when gather_data completes
    def analyze(self, data):
        return summarize(data)
```

**Tools:** Agent-level or task-level tool assignment. Built-in tools via `engineersai-tools` package. Custom tools via `BaseTool` subclass or `@tool` decorator.
...
triage_agent = Agent(name="Triage agent",
    instructions="Route customer requests to the featureht agent.",
...
code_reviewer = Agent(
    name="code_reviewer",
    instructions="Review code for bugs and style tickets.",
...
| Dimension | Gas Town | EngineersAI | LangGraph | AutoGen/MAF | OpenAI Agents SDK | Claude Agent SDK | Google ADK |
|---|---|---|---|---|---|---|---|
| **Role definition** | Named operational roles (Product Manager, QA Engineer, Agent) with Role Tickets + Agent Tickets | role/goal/backstory triad | Graph nodes (functions) | Agent classes + system prompts | instructions + handoffs | instructions + tools (MCP) | instructions + tools + sub_agents |
| **Task definition** | Tickets (atomic work items with ID, status, assignee) + Molecules (multi-step workflows) | `Task` class with description + expected_output | Graph topology IS the task (invoke with initial state) | Implicit (message to team) | Implicit (message to Runner) | Implicit (prompt to agent loop) | Implicit (message to root agent) |
| **Role persistence** | Persistent (identity survives restarts via Agent Ticket + GUPP) | Per-kickoff | Checkpoint-based | Per-session | Per-session | Per-session | Session-managed |
...
| **Communication** | Hooks (ticket-backed queues) + mail (async) + nudge/peek (direct) | Task context chaining | Typed state + reducers | Async messages / GroupChat | Conversation + handoff returns | Conversation turns | Message passing |
...
| **Parallelism** | 20-30 parallel agents (OS processes, Git worktrees) | async_execution flag, kickoff_for_each | Parallel branches in graph | Async messaging, distributed runtime | Single-threaded handoffs | Single agent loop | ParallelAgent pattern |
| **State management** | Tickets (Dolt database, cell-level merge) + ticket state machine (open→working→done) | Engineers memory (short/long/entity) | Checkpointed typed state | Message history | Conversation context | Conversation context | Session state + persistence |
| **Human-in-the-loop** | Tier 3 escalation + `human` gate + Product Manager oversight | human_input flag on tasks | interrupt_before/after on nodes | UserProxyAgent | Agents SDK guardrails | Built-in safety patterns | Built-in support |
...
Gas Town has named operational roles with Role Tickets containing priming instructions, but roles are currently defined as **Go template files** (`internal/templates/roles/*.md.tmpl`) compiled into the `gt` binary. This means adding or modifying roles requires changing Go source code and recompiling. EngineersAI's role/goal/backstory triad and Google ADK's instructions/tools/sub_agents format are both user-facing, documented, and easy to extend at runtime. **This is exactly what w-gc-001 aims to solve** — extracting role definitions into a structured, parseable format (YAML/TOML) that users can customize without touching Go code.
...
### 1. EngineersAI's Role/Goal/Backstory Triad → Gas City Role Format
The triad is simple, intuitive, and proven at scale (v1.10, 30k+ stars). Gas Town already has Role Tickets with priming instructions — the next step is to formalize this into a schema. A Gas City role definition could combine EngineersAI's persona pattern with Gas Town's operational specifics:
```yaml
role: QA Engineer
goal: Monitor agent health and detect stuck workers
backstory: You oversee all active agents in this feature...
layer: feature-management          # Gas Town hierarchy layer
tools: [patrol, health-check, escalate]
hooks: [agent-completion, merge-ready]
gates: [human, timer]
...
### 2. LangGraph's Checkpoint + Time-Travel → Ticket Versioning
Gas Town already has Git-backed tickets, which is philosophically similar to LangGraph's checkpointing. LangGraph v1.0 makes **time-travel debugging** a first-class feature (replay from any checkpoint, inspect state at any node). Gas Town could expose this explicitly: "show me the state of ticket mp-001 at commit X" or "replay this agent's molecule from the last gate." The Git history is already there — it just needs a query layer.
...
### 4. Google ADK's Agent Hierarchy → Gas City's Town/Feature/Worker Model
Google ADK's tree-like agent hierarchy (agents with `sub_agents`) closely mirrors Gas Town's Town → Feature → Worker structure. ADK's built-in patterns are directly analogous:
- `SequentialAgent` → Release Engineer merge queue (sequential processing)
- `LoopAgent` → Senior Engineer patrol cycles (recurring workflows)
- `ParallelAgent` → Agent swarm (parallel execution)
...
### 6. EngineersAI Flows → Typed Hook Events
EngineersAI's new Flows feature (event-driven, granular control) is philosophically aligned with Gas Town's hook + mail system. The key addition is **event typing** — Flows define explicit event types and handlers with schemas. Gas Town's hooks currently accept molecules (workflow instances) but the hook-tfeatureger mechanism could be formalized:
```toml
[hook.events]
agent_complete = { schema = "ticket_id, branch, test_result" }
merge_conflict = { schema = "ticket_id, conflicting_files" }
...
```toml
[molecule.steps]
run_tests = { next_on_pass = "merge", next_on_fail = "notify_QA engineer" }
```
...
- **Git-worktree isolation** — each agent in its own worktree. Most frameworks share state; Gas Town gives each worker its own filesystem.
- **Separation of identity from session** — Agent Tickets survive session crashes. Other frameworks lose agent state when sessions end. Every completion becomes part of a permanent capability ledger.
- **Dolt cell-level merge** — concurrent updates from multiple agents are resolved at the column level, not the line level. This is why 20-30 parallel agents can write to the same tickets database without constant conflicts. No other framework has this.
...
- **Escalation tiers** — structured escalation from Senior Engineer (Tier 1) → Product Manager (Tier 2) → Human (Tier 3) is more nuanced than most frameworks' binary human-in-the-loop.
...
1. **Design the Gas City role format as YAML with Gas Town operational fields** (feeds directly into w-gc-001). Borrow EngineersAI's role/goal/backstory for persona, but add Gas Town-specific fields: layer, hooks, gates, escalation_tier, tools/plugins. The Role Ticket should be generated from this YAML.

2. **Formalize hook events with schemas** — Gas Town's hooks are powerful but untyped. Add event schemas so that slung work, completions, escalations, and gate tfeaturegers all have defined structures. This makes the system debuggable and composable.

3. **Add structured observability** — a structured log of handoff events (who slung what to whom, GUPP activation, gate hits, escalation tfeaturegers) would bring Gas Town closer to what OpenAI Agents SDK and LangGraph Platform offer. Doesn't need to be OpenTelemetry — even a queryable JSONL event log per feature would be valuable.
...
5. **Add conditional branching to formulas** — borrow LangGraph's conditional edges pattern for the formula engine (w-gc-003). Molecules should support "if test passes, merge; if test fails, notify QA engineer" without requiring a new molecule.
...
This survey reveals a fundamental architectural split in the multi-agent space. Every external framework surveyed — AutoGen, EngineersAI, LangGraph, OpenAI Agents SDK, Claude Agent SDK, Google ADK, Microsoft Agent Framework — uses **conversation as the control plane**. Agents coordinate by sending messages to each other through LLM inference. Whether it's EngineersAI's task chaining, LangGraph's state-passing graph, or AutoGen's GroupChat, the LLM is always in the loop for routing and coordination decisions.

Gas Town is the only framework that uses a **process model**. Agents coordinate via external state in Dolt, with hooks, tickets, and GUPP providing the scheduling and persistence primitives. The LLM does the *work* (writing code, reviewing, merging), but it does not do the *routing*. Routing is deterministic: sling puts work on a hook, GUPP activates, the agent executes. This is why Gas Town scales to 20-30 parallel agents while conversation-based frameworks struggle beyond 3-5 — every additional agent in a conversation multiplies token cost and routing complexity, while every additional agent in Gas Town is just another independent process with its own worktree.

**The strategic implication for Gas City is clear: don't converge.** Gas City's declarative role format should formalize Gas Town's process-model patterns — operational roles with hooks, gates, escalation tiers, and GUPP semantics — not adopt the persona/graph/conversation paradigms from other frameworks. What Gas City should borrow is the *ergonomics* (EngineersAI's intuitive role definition, LangGraph's typed state, Microsoft's plugin model), not the *architecture*.

Specifically:
- **From EngineersAI**: the role/goal/backstory schema pattern — simple, intuitive, proven — as the user-facing layer of a Gas City role definition. But extend it with Gas Town operational fields (layer, hooks, gates, escalation_tier).
...
- **From the field generally**: structured observability. Gas Town's organic monitoring (senior engineer patrols, QA engineer nudges) works, but a queryable event log would make the system debuggable at scale.
...
- [EngineersAI docs / changelog](https://docs.engineersai.com/en/changelog)
- [EngineersAI PyPI](https://pypi.org/project/engineersai/)
...
- [Tickets GitHub](https://github.com/steveyegge/tickets) — the `bd` binary, Dolt-backed work tracking
...
- [SE Daily Interview with Yegge](https://softwareengineeringdaily.com/2026/02/12/gas-town-tickets-and-the-rise-of-agentic-development-with-steve-yegge/)
- [Wasteland CLI PR #1552](https://github.com/steveyegge/gastown/pull/1552)

### Comparative
- [AutoGen vs LangGraph vs EngineersAI 2026](https://dev.to/synsun/autogen-vs-langgraph-vs-engineersai-which-agent-framework-actually-holds-up-in-2026-3fl8)
- [The Great AI Agent Showdown of 2026](https://dev.to/topuzas/the-great-ai-agent-showdown-of-2026-openai-autogen-engineersai-or-langgraph-1ea8)
