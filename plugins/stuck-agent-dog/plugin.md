+++
name = "stuck-polecat-dog"
description = "Context-aware stuck/crashed polecat detection and restart for polecats and deacons"
version = 1
...
[tracking]
labels = ["plugin:stuck-polecat-dog", "category:health"]
digest = true
...
plugin checks whether an polecat is truly unresponsive before restarting.

**Design principle**: The daemon should NEVER kill workers. It detects and logs.
This plugin (running as a Dog polecat with AI judgment) makes the restart decision
...
## Step 1: Enumerate polecats to check

Gather all polecats and the deacon session. We check both crashed sessions
(session dead, work on hook) and stuck sessions (session alive but polecat hung).

```bash
echo "=== Stuck Polecat Dog: Checking polecat health ==="
...
- Its tmux session is dead OR the polecat process is dead
...
while IFS='|' read -r RIG PREFIX; do
  ...
  for PCAT_PATH in "$POLECAT_DIR"/*/; do
    ...
    if ! tmux has-session -t "$SESSION_NAME" 2>/dev/null; then
      ...
      if [ -n "$HOOK_BEAD" ]; then
        # Check polecat_state to avoid false alerts for intentional shutdowns
        AGENT_STATE=$(bd show "$RIG/polecats/$PCAT_NAME" --json 2>/dev/null \
          | jq -r '.polecat_state // empty' 2>/dev/null)
        if [ "$AGENT_STATE" = "spawning" ]; then
          echo "  SKIP $SESSION_NAME: polecat_state=spawning (sling in progress)"
          continue
        fi
        if [ "$AGENT_STATE" = "done" ] || [ "$AGENT_STATE" = "nuked" ]; then
          echo "  SKIP $SESSION_NAME: polecat_state=$AGENT_STATE (intentional shutdown, not a crash)"
          continue
        fi
      ...
      # Session alive — check for polecat process liveness
      ...
      # Check if polecat process is running in the session
      PANE_PID=$(tmux list-panes -t "$SESSION_NAME" -F '#{pane_pid}' 2>/dev/null | head -1)
      if [ -n "$PANE_PID" ]; then
        # Check if Claude or another polecat process is a descendant
        AGENT_ALIVE=$(pgrep -P "$PANE_PID" -f 'claude|node|anthropic' 2>/dev/null | head -1)
        if [ -z "$AGENT_ALIVE" ]; then
          # Polecat process dead but session alive — zombie session
          HOOK_BEAD=$(bd show "$RIG/polecats/$PCAT_NAME" --json 2>/dev/null \
            | jq -r '.hook_bead // empty' 2>/dev/null)
          if [ -n "$HOOK_BEAD" ]; then
            STUCK+=("$SESSION_NAME|$RIG|$PCAT_NAME|$HOOK_BEAD|polecat_dead")
            echo "  ZOMBIE: $SESSION_NAME (polecat dead, session alive, hook=$HOOK_BEAD)"
          fi
        else
          HEALTHY=$((HEALTHY + 1))
        fi
...
polecat, inspect the tmux pane context to determine if restart is appropriate.
...
**You (the dog polecat) must evaluate each case:**

For CRASHED polecats (session dead, work on hook):
...
For STUCK polecats (session alive, polecat dead):
- Kill the zombie session, then restart
- Exception: if pane output shows the polecat is in a long-running build/test
...
**Decision framework:**
1. If polecat is clearly dead (no process, no output) → restart
2. If polecat shows recent activity in pane → nudge first, check again next cycle
3. If polecat has been stuck for >15 minutes with no pane activity → restart
4. If mass death detected (>3 crashes in same cycle) → escalate, don't restart

## Step 5: Take action

For each polecat requiring restart:
...
for ENTRY in "${CRASHED[@]}"; do
  ...
  gt mail send "$RIG/witness" \
    ...
    --stdin <<BODY
Polecat $PCAT crash confirmed by stuck-polecat-dog plugin.
Context-aware inspection completed — polecat is genuinely dead.
...
for ENTRY in "${STUCK[@]}"; do
  ...
  gt mail send "$RIG/witness" \
    ...
    --stdin <<BODY
Polecat $PCAT zombie session cleared by stuck-polecat-dog plugin.
Session was alive but polecat process was dead.
...
if [ -n "$DEACON_ISSUE" ]; then
  echo "Escalating deacon bead: $DEACON_ISSUE"
  gt escalate "Deacon $DEACON_ISSUE detected by stuck-polecat-dog" \
    -s HIGH \
    --reason "Deacon bead: $DEACON_ISSUE. Context inspection completed."
fi
```

## Step 6: Mass death check

If multiple polecats crashed in the same cycle, this may indicate a systemic
bead (Dolt outage, OOM, etc.). Escalate instead of blindly restarting all.

```bash
TOTAL_ISSUES=$(( ${#CRASHED[@]} + ${#STUCK[@]} ))
if [ "$TOTAL_ISSUES" -ge 3 ]; then
  echo "MASS DEATH: $TOTAL_ISSUES polecats down in same cycle — escalating"
  gt escalate "Mass polecat death: $TOTAL_ISSUES polecats down" \
    -s CRITICAL \
    --reason "stuck-polecat-dog detected $TOTAL_ISSUES polecats down simultaneously.
Crashed: ${CRASHED[*]}
Stuck: ${STUCK[*]}
This may indicate a systemic bead (Dolt, OOM, infra). Investigate before mass restart."
fi
```
...
```bash
SUMMARY="Polecat health check: ${#CRASHED[@]} crashed, ${#STUCK[@]} stuck, $HEALTHY healthy"
if [ -n "$DEACON_ISSUE" ]; then
...
On success (no beads or beads handled):
```bash
bd create "stuck-polecat-dog: $SUMMARY" -t chore --ephemeral \
  -l type:plugin-run,plugin:stuck-polecat-dog,result:success \
  -d "$SUMMARY" --silent 2>/dev/null || true
```

On failure:
```bash
bd create "stuck-polecat-dog: FAILED" -t chore --ephemeral \
  -l type:plugin-run,plugin:stuck-polecat-dog,result:failure \
  -d "Polecat health check failed: $ERROR" --silent 2>/dev/null || true

gt escalate "Plugin FAILED: stuck-polecat-dog" \
  --severity high \
  --reason "$ERROR"
```
